...
class IPADetector(BaseDetector):
    ...
    def detect(self, ctx: DetectionContext) -> List[SignalResult]:
        ...
        # v79.1 MOD-100: Relaxed EQS Gate - allow 0 if DER > 0.35 (strong flow)
        eqs_total = eqs.total if hasattr(eqs, 'total') else 0
        der_val = ctx.snapshot.der if hasattr(ctx.snapshot, 'der') else 0.0
        
        is_strong_flow = abs(der_val) > 0.35
        if eqs_total < 0 or (eqs_total == 0 and not is_strong_flow):
            self.last_reject_reason = f'EQS_TOTAL {eqs_total} too low (no institutional conviction or poor entry quality)'
            return []
