...
@dataclass
class RegimeResult:
    """Result of regime detection. v27.1: WEAKENING regime + DI fields. atr_m5 moved to MarketSnapshot."""
    regime: str           # 'TRENDING' | 'WEAKENING' | 'RANGING' | 'VOLATILE' | 'DEAD'
    ...
    def __str__(self) -> str:
        return (f"Regime: {self.regime} | ADX:{self.adx_h1:.1f} | "
...
class MarketRegimeDetector:
    ...
    def detect(self, candles_m5: pd.DataFrame,
                candles_h1: Optional[pd.DataFrame]) -> RegimeResult:
        """
        ...
        try:
            ...
            # 3. Classify regime (v27.0: include WEAKENING)
            regime = self._classify_regime(adx_h1, bb_width, di_spread, atr_ratio)
    ...
    def _classify_regime(self, adx: float, bb_width: float, di_spread: float = 0, atr_ratio: float = 1.0) -> str:
        """
        Classify market regime based on ADX, DI spread, ATR ratio, and BB width.
        v27.0: Added WEAKENING regime.
        v30.6: BB thresholds calibrated for BTC M5 (was Forex defaults)

        Decision tree (BTC-calibrated):
          WEAKENING:    ADX > 40 + DI spread < 10 + ATR ratio < 0.75
          TRENDING:     ADX > 40 (strong, but not weakening)
          ADX > 20:
        ...
        if adx > self.adx_strong_threshold:
            ...
            if bb_width < self.bb_dead_threshold:
                return 'DEAD'
            else:
                return 'RANGING'
